...
- ✅ E2Eテスト品質改善（フレーキーパターン修正）
- ✅ 技術的負債解消（Issue #70, #71 確認完了）
...
| ファイル | 内容 |
|---------|------|
| `documents/規約優先順位ガイド.md` | **規約の優先順位と自動チェック**（重要） |
...
PRを作成すると、`.github/workflows/code-standards.yml`により以下がチェックされます：

| チェック項目 | 内容 | 現在の状態 |
|-------------|------|-----------|
| **E2Eセレクタ規約** | `getByText`/`:has-text`の使用検出 | ⚠️ 警告 |
| **APIバリデーション** | Zodバリデーション有無 | ⚠️ 警告 |
| **Gherkinシナリオ** | 対応するfeatureファイルの存在 | ⚠️ 警告 |
| **data-testid属性** | インタラクティブ要素への付与 | ⚠️ 警告 |
| **tenant_idフィルタ** | Prismaクエリでのフィルタ有無 | ⚠️ 警告 |

**注意**: 現在は警告のみですが、既存コード修正後にCI失敗に変更予定。

### 規約優先順位（重要度順）

**CRITICAL（必須・CI失敗）**:
1. TypeScript型安全性（`any`禁止）
2. ESLintエラー0件
3. テスト通過

**HIGH（強く推奨・CI警告）**:
4. E2Eテストで`data-testid`セレクタ使用
5. APIルートでZodバリデーション
6. tenant_idフィルタリング
...
7. Page Objectパターン使用
8. Gherkinシナリオ先行開発
9. コンポーネントにdata-testid追加

**詳細**: `documents/規約優先順位ガイド.md` を参照

### AIエージェントへの重要な指示

**既存コードを参考にする際の注意**:

既存コードには規約違反のパターンが残っています。以下のパターンは**使用禁止**です：

```typescript
// ❌ 禁止パターン（既存コードにあっても使わない）
await page.getByText('予約を確定する').click();
await page.locator('button:has-text("予約")').click();

// ✅ 正しいパターン（新規コードはこちらを使用）
await page.locator('[data-testid="submit-booking"]').click();
```

**新規コード作成時のルール**:
1. 必ず`data-testid`セレクタを使用
2. APIルートには必ずZodバリデーションを追加
3. Prismaクエリには必ず`tenant_id`フィルタを追加
